...
export default function AdminDashboard() {
  ...
  const { 
    ...
    requireAdmin: true,  // Redirect ke dashboard jika bukan admin/rt/ketua_rt
  });
  ...
  // Fetch tickets saat auth selesai
  useEffect(() => {
    if (!isCheckingAuth) {
      fetchTickets();
    }
  ...
  const filteredData = tickets.filter(t => {
    ...
    const matchSearch = t.title.toLowerCase().includes(searchLower) || 
                        ...
                        (t.user?.full_name && t.user.full_name.toLowerCase().includes(searchLower));
                                ...
                                <tr key={t.id} className="hover:bg-slate-50 transition-colors">
                                    <td className="px-6 py-4 align-top">
                                        ...
                                        <div className="font-bold text-slate-800">
                                            {t.user?.full_name || t.user?.email || 'Anonim'}
                                        </div>
                                        {t.user?.blok_rumah && (
                                          <div className="text-xs text-slate-400">Blok {t.user.blok_rumah}</div>
                                        )}
                                    </td>
